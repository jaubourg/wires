...
function displayHelpAndExit( exitCode ) {
	console.log( fs.readFileSync( __dirname + "/../data/help.txt" ).toString().replace( /ENGINE/g, engineName ) );
...
for ( var i = 0; !targetScript && i < length; i++ ) {
	...
	if ( rCommand.test( arg ) ) {
		...
		if ( notSupported[ arg ] ) {
			error( engineName + " option not supported (" + Object.getOwnPropertyNames( notSupported ).join( ", " ) + ")" );
		}
		if ( wiresOptions[ arg ] ) {
		...
		// let's keep the overrides for later
		overrides.push( arg );
	} else {

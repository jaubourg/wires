        ...
        if ( isDir ) {
            ...
            if ( isComputed ) {
                if ( typeof value === `string` ) {
                    ...
                    value = ( ...items ) => {
                        if ( !func ) {
                            const modulePath = require.resolve( `${ this.parseRoute( _value, directory ) }` );
                            func = require( modulePath );
                            if ( typeof func !== `function` ) {
                                throw new Error( `route ${ _key } = <<${ _value }>> is not a function` );
                            }
                            funcConfig = Config.cache( dirname( modulePath ) );
                        }
                        return funcConfig.getRoute( func( ...items ) );
                    };
                } else {
                    value = ( ...items ) => this.getRoute( _value( ...items ) );
                }
            }

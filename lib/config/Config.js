...
Config.prototype = {
    ...
    "handleExpression": function( expr, requester, resolveFilename ) {
        ...
        if ( ( expr[ 0 ] === "#" ) || ( expr[ 0 ] === "?" ) ) {
            ...
            filename = ( expr[ 1 ] === ">" ? "" : ( this._module ? this._module.filename : this._directory ) ) + expr;
            if ( !Module._cache[ filename ] ) {
                module = new Module( filename, this._module || requester );
    ...
    "parse": function( expr, directory, isRoute ) {
        ...
        var self = this;
        var tmp;
        expr = expr == null ? expr : parse( String( expr ), function( __, modifier, subExpr ) {
    ...
    "dirRoute": function( expr ) {
        ...
        return found && ( found.value + expr.slice( found.i ).join( "/" ) );
    },

...
Config.prototype = {
	...
	parse: function( expr, directory, isRoute ) {
		...
		if ( isRoute && ( tmp = rHere.exec( expr ) ) ) {
			expr = path.resolve( tmp[ 1 ] === "./" ?  ( directory || this._directory ) : process.cwd(), tmp[ 2 ] );
		}
		return expr;
	},
	...
	route: function( expr, requester ) {
		...
		if ( this._routes && this._routes[ expr ] ) {
			return Module._resolveFilename(
				requester.parse( this._routes[ expr ], this._directory, true ),
				this._module
			);
		}
		if ( this._dirRoutes && rPath.test( expr ) && ( value = this.dirRoute( expr ) ) ) {
			return Module._resolveFilename(
				requester.parse( value, this._directory, true ),
				this._module
			);
		}
		if ( this._parent ) {

...
const rCast = /^\(\s*(bool(?:ean)?|num(?:ber)?)\s*\)\s*/;
const rDir = /\/(\(\))?$/;
const rFallback = /\?$/;
const rHere = /^(\.\.?|[>~])\/(.*)$/;
...
const isBad = value => ( value == null ) || ( value === `` ) || ( ( typeof value === `number` ) && isNaN( value ) );

const cast = ( string, type ) => {
    const first = type[ 0 ];
    if ( first === `b` ) {
        const tString = string.trim();
        return ( tString === `true` ) || ( tString === `false` ? false : null );
    }
    if ( first === `n` ) {
        const tString = string.trim();
        return tString ? Number( tString ) : NaN;
    }
    return string;
};
...
class Config {
    ...
    parseData( data ) {
        if ( data ) {
            if ( typeof data === `string` ) {
                let type = `string`;
                const value = this.parse( data.replace( rCast, ( _, t ) => {
                    type = t;
                    return ``;
                } ) );
            ...
            if ( isObject( data ) ) {
                ...
                for ( const key of fallbacks ) {
                    ...
                    if ( !output.hasOwnProperty( realKey ) || isBad( output[ realKey ] ) ) {
    ...
    getRoute( expr ) {
        ...
        if ( value === null ) {
            return require.resolve( `./util/null` );
        }
        return this.parseRoute( value, route.directory );
    }
